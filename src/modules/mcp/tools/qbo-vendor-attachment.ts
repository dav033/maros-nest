...
export function registerQboVendorMatchingTools(
  server: McpServer,
  deps: McpToolDeps,
) {
  const resolveQboRealmId = (realmId?: string) => resolveRealmId(deps, realmId);
  ...
  registerMcpTool(
    ...
    async ({ realmId }: { realmId?: string }) => {
      ...
      return deps.qboVendorMatching.listQboVendors(rid);
    },
  );
  ...
  registerMcpTool(
    ...
    async ({
      ...
      const rid = await resolveQboRealmId(realmId);
      return deps.qboVendorMatching.matchCrmCompaniesToVendors(rid, options);
    },
  );
  ...
  registerMcpTool(
    ...
    async ({ companyId, realmId }: { companyId?: number; realmId?: string }) => {
      const rid = await resolveQboRealmId(realmId);
      return deps.qboVendorMatching.suggestVendorMatches(rid, companyId);
    },
  );
  ...
  registerMcpTool(
    ...
    async ({ realmId }: { realmId?: string }) => {
      const rid = await resolveQboRealmId(realmId);
      return deps.qboVendorMatching.getVendorCrmMap(rid);
    },
  );
}
...
export function registerQboAttachmentTools(server: McpServer, deps: McpToolDeps) {
  ...
  registerMcpTool(
    ...
    async (params: {
      ...
      realmId?: string;
    }) => deps.qboAttachments.getProjectAttachments(params),
  );
  ...
  registerMcpTool(
    ...
    async ({
      ...
      const rid = await resolveQboRealmId(realmId);
      return deps.qboAttachments.getAttachmentsForEntity(rid, entityType, entityId, {
        includeTempDownloadUrl,
      });
    },
  );
  ...
  registerMcpTool(
    ...
    async ({ attachableId, realmId }: { attachableId: string; realmId?: string }) => {
      const rid = await resolveQboRealmId(realmId);
      return deps.qboAttachments.getAttachmentDownloadUrl(rid, attachableId);
    },
  );
}
